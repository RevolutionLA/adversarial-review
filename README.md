...
真实产出：本仓库的 v2.0 就是对 v1.1 跑了一轮重档评审后的整改结果。
...
- **v2.0 第三方**：整改文档里的"已修复"声明**是假的**（用 blob 哈希证伪）；CI 有个步骤**名实不符**；新增的测试有 5 个突变逃逸。
- **v2.0.2 中立裁定**：**修 bug 的过程引入了新 bug**——`>` 与 `|` 的解析语义被对调，而那个提交**自带测试是红的却照常提交了**；安装验证脚本**向用户报告假的"验证通过"**。

**最后一轮最值得记住**：裁定方发现我提交的版本自测 3 条失败却照样提交——原因是我当时在一个**被测试污染的文件**上做验证，看到了假的"20 条全过"。

**这正是本 skill 开篇点名的缺陷类型（为了修 A 而引入 B），而且发生在一个专门抓这类缺陷的工具上。**

于是有了 v2.0.2 的 **`.githooks/pre-commit` 门禁**：回归测试 + 突变测试 + 规范校验 + 链接检查，任一失败即拒绝提交。**已用破坏性测试验证过它真能拦住**——若当初有它，那个坏提交根本进不来。

> **一个诚实的说明**：这个 skill 不能保证你不犯错。它能保证的是——**错误会被暴露出来，而不是被掩盖**。上面两轮里最严重的两个缺陷，都是我写的，也都是被这个流程自己抓出来的。
