...
## 这个 skill 是拿自己验证过的（包括被自己打脸）

三轮自评审的实际战绩，**完整记录在 `docs/review/`，包括我自己的失误**：

| 轮次 | 角色 | 抓到什么 |
|---|---|---|
| v1.1 | 自评审 | 第 3 步让蓝军自己裁定——**当事人当法官**，与自己的核心规则冲突 |
| v2.0 | 蓝军（15 条） | 校验器把 `description: >` 读成 1 字符**却报"通过"**；SKILL.md 零处引用配套文件；**不符合 skills.sh 目录结构所以不会被索引**；CI 链接检查**永远不可能失败**；本机装的还是旧版 |
| v2.0 | 第三方 | **整改文档虚假声明**（用 blob 哈希证伪）；CI step 名实不符；测试有 5 个突变逃逸 |
| v2.0.2 | 中立裁定 | **我的修复引入了 B1 的镜像复发**——`>` 与 `\|` 语义对调，而该提交自带测试是红的（17/3）；`verify-install.sh` **向用户报假的"验证通过"**；新写的 `check-links.mjs` **自己就是静默通过** |

**最后一轮最值得看**：裁定方在隔离副本上运行，发现我提交的版本**自测 3 红却照常提交**——因为我当时是在一个被测试污染的文件上验证的，看到了假的"20 通过"。

**这正是本 skill 开篇点名的缺陷类型（为了修 A 引入 B），而且发生在一个专门抓这类缺陷的工具上。**

结果就是 v2.0.2 引入的 **`.githooks/pre-commit` 门禁**：回归测试 + 突变测试 + 规范校验 + 链接检查，任一失败即禁止提交。**已破坏性验证过它真能拦截**——若当初有它，那个坏提交根本进不来。

> **一个诚实的说明**：这个 skill 不能保证你不犯错。它能保证的是——**错误会被记录下来，而不是被掩盖**。上面三轮里，有两条最严重的缺陷是我自己写的、又是被这个流程自己抓出来的。
